...
void setup() {
  ...
  **/
   ...
   */
  myRemoteLocation = new NetAddress("127.0.0.1",8004);
...
val4 = theOscMessage.get(3).floatValue();
...
//    oscP5.send(myMessage, myRemoteLocation);
  ...
  // Draw grey lines dividing the screen into quadrants.
    ...
    text(str(val1), video.width/4, video.height/4);
    text(str(val2), 3*video.width/4, video.height/4); 
    text(str(val3), video.width/4, 3*video.height/4); 
    text(str(val4), 3*video.width/4, 3*video.height/4);  
